...
export function JobList({ title, jobs, mainJob, jobLevels, lang, type, pointColor }: JobListProps) {
    ...
    return (
        <div>
            <div className="text-[10px] text-neutral-400 dark:text-[#86868b] font-semibold uppercase tracking-wider mb-2">{title}</div>
            <div className="flex flex-wrap gap-2">
                {displayJobs.map(job => (
                    ...
                    >
                        ...
                        <span className="text-xs font-medium whitespace-nowrap text-neutral-700 dark:text-[#d1d1d6]">
                            {getJobName(job)}
                        </span>
                        <span className="text-[10px] font-semibold whitespace-nowrap" style={{ color: pointColor }}>
                            Lv.{jobLevels[job.id] || '?'}
                        </span>
                    </div>
                ))}
